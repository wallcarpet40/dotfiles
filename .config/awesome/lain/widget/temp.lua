...
local function factory(args)
    ...
    local tempfile = args.tempfile or "/sys/class/hwmon/hwmon2/temp1_input"
    local settings = args.settings or function() end

    function temp.update()
        helpers.async({"find", "/sys/class/hwmon/hwmon2", "-type", "f", "-name", "temp1_input"}, function(f)
            temp_now = {}
            ...
            coretemp_now = temp_now[tempfile]  or "EI"
            widget = temp.widget
            settings()
        end)
    end
